...
export function connectorContext(opts: RunnerOptions, source: string): ConnectorContext {
  const { store } = opts;
  const subs = store.listSubscriptions();
  return {
    catalog: opts.catalog,
    state: store.stateFor(source),
    log: opts.log ?? noopLogger,
    now: opts.now ?? (() => new Date()),
    home: opts.home ?? homedir(),
    env: opts.env ?? process.env,
    planFor: (provider) => subs.find((s) => s.provider === provider)?.plan,
    identity: opts.identity ?? identityFromStore(store),
    fetch: opts.fetch,
  };
}

function identityFromStore(store: EventStore): ConnectorContext['identity'] {
  const email = store.getSetting('identity.email');
  const name = store.getSetting('identity.name');
  const device = store.getSetting('identity.device');
...
export async function ingest(opts: RunnerOptions, source: string, events: AsyncIterable<RawUsageEvent>): Promise<RunResult> {
  const { store, catalog } = opts;
  const started = Date.now();
  const runId = store.startRun(source);
  ...
  const flush = () => {
    ...
    const r = store.upsertEvents(batch, source);
  ...
  store.finishRun(runId, { seen: result.seen, inserted: result.inserted, updated: result.updated, unpriced: result.unpriced, error: result.error ?? undefined });
  return result;
}
...
export async function runLocalScan(opts: RunnerOptions, scan: ScanOptions = {}, only?: string[]): Promise<RunResult[]> {
  ...
  for (const c of localConnectors as LocalConnector[]) {
    ...
    const ctx = connectorContext(opts, c.id);
    ...
    const r = await ingest(opts, c.id, c.scan(ctx, scan));
...
export async function runApiPull(opts: RunnerOptions, connector: ApiConnector, pull: PullOptions = {}, creds?: Record<string, string>): Promise<RunResult> {
  ...
  const ctx = connectorContext(opts, connector.id);
  return ingest(opts, connector.id, connector.pull(ctx, resolved, pull));
}
...
export async function runImport(opts: RunnerOptions, connector: ImportConnector, file: string, importOpts: ImportOptions = {}): Promise<RunResult> {
  const ctx = connectorContext(opts, connector.id);
  return ingest(opts, connector.id, connector.import(ctx, file, importOpts));
}
